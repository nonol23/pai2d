...
// msg sizes
#define HELLO_MSG_SIZE 30 // number of bytes. MANDATORY: must be consistant with message_template
#define ACK_MSG_SIZE 30
#define OK_MSG_SIZE 14
#define CHECK_MSG_SIZE 14
...
typedef struct connecting_robot {
	...
	int partner_msg_id;
    double concentration_value;
    int count_robots; // valeur de robots compter à l'aide de la concentation 
    bool same_count; //true si même compte que ses voisins else false 
} connecting_robot;
...
void send_message(uint8_t msg_type, int hello_msg_id, int recipient_id, int partner_msg_id, int my_id, uint8_t probe_attempt, double concentration_value, int count_robots);
...
void send_message(uint8_t msg_type, int hello_msg_id, int recipient_id, int partner_msg_id, int my_id, uint8_t probe_attempt, double concentration_value, int count_robots)
{
    ...
    my_message.values.count_robots = count_robots;
...
void clear_connecting_neighbour(connecting_robot *robot){
    ...
    robot->count_robots = 0;
    robot->same_count = false; 
}
    ...
    */
    ...
    while (1) {
        ...
        if (sending_HELLO){   
            if (send){
#ifndef CLEAN
                send_message(HELLO, -1, -1, my_id, probe_attempt, concentration_value, count_robots);
#else
                send_message(HELLO, hello_msg_id, -1, -1, my_id, 0, concentration_value, count_robots);
#endif
        ...
        if (sending_ACK){   
            if (send){
#ifndef CLEAN
                send_message(ACK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, probe_attempt, concentration_value, count_robots);
#else
                send_message(ACK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, 0, concentration_value, count_robots);
#endif
        ...
        if (sending_OK){    
            if (send){
#ifndef CLEAN
                send_message(OK, my_partner.robot_id, -1, my_id, probe_attempt, concentration_value, count_robots);
#else
                send_message(OK, hello_msg_id, my_partner.robot_id, -1, my_id, 0, concentration_value, count_robots);
#endif
        ...
        if(sending_CHECK){
            if (send){
#ifndef CLEAN
                send_message(CHECK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, probe_attempt, concentration_value, count_robots);
#else
                send_message(CHECK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, 0, concentration_value, count_robots);
#endif
                if (DEBUG%2){
                    printf("[%d] sent CHECK %d, %d\n", my_id, my_partner.robot_id, hello_msg_id);
                }
            }
        }
        ...
        if ( ( BANSHEE_VER > 0 ) || !(sending_OK) ) // read FIFO buffer - any message(s)?
        {
            ...
            while ( pogobot_infrared_message_available() && nb_msg < MAX_NB_OF_MSG )
            {
                ...
                */
                ...
                // 4) i receive a CHECK -> check the concentration between them, if same set same_count at true else false
                ...
                if (sending_HELLO){
                    if (received_message.values.msg_type == HELLO){
                        ...
                        my_partner.count_robots = received_message.values.count_robots;
                        sending_ACK = 1;
                        sending_OK = 0;
                        sending_HELLO = 0;
                        msg_wait_step = 0;
                ...
                if (received_message.values.msg_type == CHECK) {
                    if(my_partner.robot_id == -1) 
                        my_partner.robot_id = sender_id;
                    ...
                    // on vérifie si les comptes son égaux 
                    if(my_partner.count_robots == count_robots){
                        my_partner.same_count = true;
                        printf("%d, same count \n", count_robots);
                    } else {
                        my_partner.same_count = false;
                        printf("not the same count \n ");
                    }
                    continue;  // on ne traite pas ce message comme un message de handshake
                }
        ...
        if (communication_established){
            ...
            count_robots = (int)(1.0 / concentration_value); 

            static int cpt_check = 0;
            if (++cpt_check >= 10) { //tout les 10 échanges on vérifie 
                send_message(CHECK, hello_msg_id, -1, -1, my_id, 0, concentration_value, count_robots);
                cpt_check = 0;
            }
