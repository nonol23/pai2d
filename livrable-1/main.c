...
// msg sizes
#define HELLO_MSG_SIZE 26 // number of bytes. MANDATORY: must be consistant with message_template
 ...
 */
#define robot_idA 31022	// 26: 2349     13: 22571   3: 21547	38: 38958   18: 32812   189: 36140  47: 31275   148: 39726  128: 18216
...
void send_message(uint8_t msg_type, int hello_msg_id, int recipient_id, int partner_msg_id, int my_id, uint8_t probe_attempt, double concentration_value);
    ...
    */
    ...
    while (1) {
        ...
        if (sending_HELLO){   
            if (send){
#ifndef CLEAN
                send_message(HELLO, -1, -1, my_id, probe_attempt, concentration_value);
#else
                send_message(HELLO, hello_msg_id, -1, -1, my_id, 0, concentration_value);
#endif
        ...
        if (sending_ACK){   
            if (send){
#ifndef CLEAN
                send_message(ACK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, probe_attempt, concentration_value);
#else
                send_message(ACK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, 0, concentration_value);
#endif
        ...
        if (sending_OK){    
            if (send){
#ifndef CLEAN
                send_message(OK, my_partner.robot_id, -1, my_id, probe_attempt, concentration_value);
#else
                send_message(OK, hello_msg_id, my_partner.robot_id, -1, my_id, 0, concentration_value);
#endif
