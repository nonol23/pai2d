...
// msg sizes
#define HELLO_MSG_SIZE 34 // number of bytes. MANDATORY: must be consistant with message_template
#define ACK_MSG_SIZE 34
...
typedef struct RawMessage {
    ...
    double concentration_value1;
    double concentration_value2;
} RawMessage;
...
void send_message(uint8_t msg_type, int hello_msg_id, int recipient_id, int partner_msg_id, int my_id, uint8_t probe_attempt, double concentration_value1, double concentration_value2);
...
void send_message(uint8_t msg_type, int hello_msg_id, int recipient_id, int partner_msg_id, int my_id, uint8_t probe_attempt, double concentration_value1, double concentration_value2)
{
    ...
    my_message.values.concentration_value1 = concentration_value1;
    my_message.values.concentration_value2 = concentration_value2;
...
void clear_connecting_neighbour(connecting_robot *robot){
    ...
    robot->concentration_value1 = 0.0;
    robot->concentration_value2 = 0.0;
}
...
int main(void){
    ...
    // diffusion
    double concentration_value1 = 0.0;
    double concentration_value2 = 0.0;
    if (my_id == robot_idA){
        concentration_value1 = 1.0;
        concentration_value2 = 1.0;
    }
    ...
    char dec_part1[NB_DIGIT+1];
    char dec_part2[NB_DIGIT+1];
    ...
    */
    ...
    while (1) {
        ...
        if (DEBUG > 1){
            set_rgb_colors(concentration_value1);
            set_rgb_colors(concentration_value2);
        }
        ...
        if (sending_HELLO){   
            if (send){
#ifndef CLEAN
                send_message(HELLO, -1, -1, my_id, probe_attempt, concentration_value1, concentration_value2);
#else
                send_message(HELLO, hello_msg_id, -1, -1, my_id, 0, concentration_value1, concentration_value2);
#endif
        ...
        if (sending_ACK){   
            if (send){
#ifndef CLEAN
                send_message(ACK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, probe_attempt, concentration_value1, concentration_value2);
#else
                send_message(ACK, hello_msg_id, my_partner.robot_id, my_partner.partner_msg_id, my_id, 0, concentration_value1, concentration_value2);
#endif
        ...
        if (sending_OK){    
            if (send){
#ifndef CLEAN
                send_message(OK, my_partner.robot_id, -1, my_id, probe_attempt, concentration_value1, concentration_value2 );
#else
                send_message(OK, hello_msg_id, my_partner.robot_id, -1, my_id, 0, concentration_value1, concentration_value2 );
#endif
                ...
                */
                ...
                if (sending_HELLO){
                    if (received_message.values.msg_type == HELLO){
                        ...
                        my_partner.concentration_value1 = received_message.values.concentration_value1;
                        my_partner.concentration_value2 = received_message.values.concentration_value2;
                        sending_ACK = 1;
                        sending_OK = 0;
                        sending_HELLO = 0;
                        msg_wait_step = 0;
        ...
        if (communication_established){
            ...
            double delta_concentration1 = KAPPA1 * ( my_partner.concentration_value1 - concentration_value1 );
            concentration_value1 += delta_concentration1;
            double delta_concentration2 = KAPPA2 * ( my_partner.concentration_value2 - concentration_value2 );
            concentration_value2 += delta_concentration2;

            effective_msg_count++;

            double_get_decimal_part_string(concentration_value1, dec_part1);
            double_get_decimal_part_string(concentration_value2, dec_part2);
            if (DEBUG==3)
                printf("%d,%d,%d,%d,%d,%d.%s,%d.%s\n", my_id, timestep, my_partner.robot_id, hello_msg_id, my_partner.partner_msg_id, double_get_integer_part(concentration_value1), dec_part1, double_get_integer_part(concentration_value2), dec_part2);
#ifndef CLEAN
            if (PROBE && my_partner.robot_id == other_robot_id && !(stop_listening)){
                ...
                double_get_decimal_part_string(total_seconds, dec_part1);
        ...
        double_get_decimal_part_string(concentration_value2, dec_part2);
        if (DEBUG==2)
            printf("%d,%d.%s,%d.%s,%d,%d\n", my_id, double_get_integer_part(concentration_value1), dec_part1, double_get_integer_part(concentration_value2), dec_part2, timestep, effective_msg_count);
